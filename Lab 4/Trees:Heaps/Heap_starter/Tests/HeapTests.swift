...
class HeapTests: XCTestCase {
  ...
  private func verifyMaxHeap(h: Heap<Int>) -> Bool {
    for i in 0..<h.count {
      let left = h.indexOfLeftChild(i: i)
      let right = h.indexOfRightChild(i: i)
      let parent = h.indexOfParent(i: i)
  ...
  func testMaxHeapOneElement() {
    ...
    XCTAssertTrue(verifyMaxHeap(h: heap))
    XCTAssertTrue(verifyMinHeap(h: heap))
    XCTAssertFalse(heap.isEmpty)
  ...
  func testCreateMaxHeap() {
    ...
    XCTAssertTrue(verifyMaxHeap(h: h1))
    XCTAssertFalse(verifyMinHeap(h: h1))
    ...
    XCTAssertTrue(verifyMaxHeap(h: h2))
    XCTAssertFalse(verifyMinHeap(h: h2))
    ...
    XCTAssertTrue(verifyMaxHeap(h: h3))
    XCTAssertFalse(verifyMinHeap(h: h3))
    ...
    XCTAssertTrue(verifyMaxHeap(h: h4))
    XCTAssertFalse(verifyMinHeap(h: h4))
  ...
  func testCreateMinHeap() {
    let h1 = Heap(array: [1, 2, 3, 4, 5, 6, 7], sort: <)
    XCTAssertTrue(verifyMinHeap(h: h1))
    XCTAssertFalse(verifyMaxHeap(h: h1))
    ...
    let h2 = Heap(array: [7, 6, 5, 4, 3, 2, 1], sort: <)
    XCTAssertTrue(verifyMinHeap(h: h2))
    XCTAssertFalse(verifyMaxHeap(h: h2))
    ...
    let h3 = Heap(array: [4, 1, 3, 2, 16, 9, 10, 14, 8, 7], sort: <)
    XCTAssertTrue(verifyMinHeap(h: h3))
    XCTAssertFalse(verifyMaxHeap(h: h3))
    ...
    let h4 = Heap(array: [27, 17, 3, 16, 13, 10, 1, 5, 7, 12, 4, 8, 9, 0], sort: <)
    XCTAssertTrue(verifyMinHeap(h: h4))
    XCTAssertFalse(verifyMaxHeap(h: h4))
  ...
  func testCreateRandomMaxHeap() {
    for n in 1...40 {
      let a = randomArray(n: n)
      let h = Heap(array: a, sort: >)
      XCTAssertTrue(verifyMaxHeap(h: h))
      XCTAssertFalse(h.isEmpty)
      XCTAssertEqual(h.count, n)
      XCTAssertTrue(isPermutation(array1: a, h.elements))
    }
  }
  ...
  func testCreateRandomMinHeap() {
    for n in 1...40 {
      let a = randomArray(n: n)
      let h = Heap(array: a, sort: <)
      XCTAssertTrue(verifyMinHeap(h: h))
      XCTAssertFalse(h.isEmpty)
      XCTAssertEqual(h.count, n)
      XCTAssertTrue(isPermutation(array1: a, h.elements))
    }
  ...
  func testInsertArrayAndRemove() {
    var heap = Heap<Int>(sort: >)
    for elem in [1, 3, 2, 7, 5, 9]{
      heap.insert(value: elem)
    }
